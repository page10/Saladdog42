...
public class TileOfTerrain : MonoBehaviour
{
    ...
    public int Cost(MoveType moveType)  // 根据某个兵种类型给出对应移动力消耗的方法 属于这个格子的功能 所以写在这个底下
    {
        foreach (MovementInfo costInfo in moveCosts)
        {
            if (costInfo.moveType == moveType)
            {
                return costInfo.moveCost;
            }
        }
